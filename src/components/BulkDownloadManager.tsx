...
export function BulkDownloadManager({ source, seriesTitle, episodes }: BulkDownloadManagerProps) {
  ...
  const episodeServersMap = new Map<number, {name: string, quality: string}[]>();
  ...
  episodes.forEach((ep) => {
    ...
    dlServers.forEach(s => allServers.add(s.name));
    
    qualitiesMap.set(ep.number, dlServers.map(s => s.quality).filter(Boolean) as string[]);
    episodeServersMap.set(ep.number, dlServers.map(s => ({name: s.name, quality: s.quality || ''})));

    let filtered = dlServers;
    if (serverPref !== 'auto') {
      filtered = dlServers.filter(s => s.name === serverPref);
    }
    
    if (filtered.length > 0) {
      let url = filtered[0].directUrl;
  ...
  const handleCopyLinks = () => {
    ...
    showToast(`Copied ${selected.size} links to clipboard.`, 'success');
  };
  ...
  const handleShareLinks = async () => {
    ...
    if (navigator.share) {
      ...
      showToast('Web Share API is not supported on this browser. Use "Copy Links" instead.', 'error');
    }
  };
        ...
        <div className="relative z-10 p-lg flex flex-col md:flex-row md:items-end justify-between gap-md">
          ...
          <div className="flex flex-col md:flex-row gap-md">
            <div className="flex flex-col gap-xs min-w-[200px]">
              <label className="font-label-caps text-outline uppercase font-bold text-[12px] tracking-wider">Preferred Server</label>
              <div className="relative">
                ...
                >
                  ...
                  {Array.from(allServers).map(s => <option key={s} value={s}>{s}</option>)}
                </select>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse whitespace-nowrap md:whitespace-normal">
            <thead>
              ...
              <th className="p-md font-label-caps text-outline uppercase tracking-wider text-[12px] font-bold">Available Servers</th>
          ...
          <tbody className="font-body-md text-body-md">
            {episodes.map(ep => {
              ...
              const serversInfoList = episodeServersMap.get(ep.number) || [];
                ...
                <tr key={ep.number} className={`border-b border-outline-variant hover:bg-surface-variant/20 transition-colors ${!url ? 'opacity-50' : ''}`}>
                  ...
                  <td className="p-md">
                    <div className="flex flex-wrap gap-xs">
                      {serversInfoList.length > 0 ? serversInfoList.map((s, i) => (
                        <span key={i} className={`px-xs py-0.5 rounded-xl font-label-caps text-[12px] font-bold uppercase ${
                          s.name === serverPref ? 'bg-primary-container text-white' : 'bg-surface-variant text-on-surface-variant'
                        }`}>
